...
* :x: [shadow](https://github.com/shadow-maint/shadow) uses struct utmp to define max. length of username. login uses utmp but is disabled in favor of util-linux. Does not seem to depend on it, only updating it.
  * **chfn**/**chsh**/**expiry**/**gpasswd**/**newgidmap**/**newgrp**/**newuidmap**/**passwd**
    * getlogin()
...
* :x: sudo
  ...
  * pututxline()
* :x: [systemd](https://github.com/systemd/systemd) includes `utmp.h` and `utmpx.h` in several places. Uses utmp for wall messages and to determine the runlevel.
  * utmpxname()
  * setutxent()
  * getutxent()
  * getutxid()
...
1. getlogin()/getlogin_r() from **glibc** itself
2. who is logged in and on which device
3. determine TTYs where wall messages should be send (not only wall, but also systemd and others)
4. determine boot time (could be extracted from journald?)
5. determine runlevel (is this really from relevance with systemd?)

The main user of utmp is **glibc** itself with getlogin()/getlogin_r(). I don't have an idea yet how to solve that, musl libc uses an environment variable, which I think is pretty unsafe.
...
Leaves, beside glibc, tools like wall or systemd, which prints messages e.g. at shutdown time to all logged in users.

# btmp/lastlog

`btmp` could be dropped, contains a random list of user names from mostly brute force attacks which could also be find in journald.
